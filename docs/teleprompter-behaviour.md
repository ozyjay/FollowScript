...
The prompt uses white rounded text on black, grouped into four-token rows in portrait and six-token rows in landscape. The next word to speak is bright yellow with a subtle background; this is a presentation-only cue one token beyond the alignment engine’s estimated spoken position. Words in the current partial recognition retain a softer yellow and lighter background as context without overriding the bright next-word cue. Dimming keeps a one-token safety buffer behind the estimate: only words at least two positions before it fade to 48% white. The estimated token and the immediately preceding token remain undimmed because partial recognition may be completing a word or briefly running ahead of the speaker. Following words remain white as look-ahead text. Users can choose whether the row containing the next-word cue is centred; when that option is off, it follows the general left or centred text-alignment preference. Once the estimated position reaches the final token, no bright cue is shown. Users can also disable highlighting and choose 28–72 point text and 4–20 point row spacing.
