...
The prompt uses white rounded text on black, grouped into eight-token rows. The current token and next three tokens form the active phrase and are yellow with a subtle background; the row containing that phrase is always centred in the reading area so the speaker's focus remains central. Earlier text fades to 48% white. Users can disable highlighting and choose 28–72 point text, 4–20 point row spacing, and left or centred alignment for the surrounding rows.
