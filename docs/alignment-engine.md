...
While tracking locally, continuity gets one additional tie-breaker. If the raw best endpoint lies beyond the normal phrase-length progression window, but a candidate within `recognised token count + 1` positions of the committed anchor scores within `0.05`, FollowScript prefers that nearby candidate. This only applies in local follow mode. Global reacquisition still chooses from the full forward search and therefore remains able to recover after a genuine omission or skipped passage.
...
With recognition window `W ≤ 16`, length tolerance `T = 4`, beam width `K = 7`, local span `B ≤ 81` and remaining tokens `N`, work is approximately `O(B × T × W²)` locally and `O(N × T × W²)` during reacquisition. Clustering and the continuity tie-breaker operate only over the retained beam, so they add constant-size work relative to candidate scoring. The score is lexical and heuristic rather than a calibrated probability. Phonetic matching is an interface seam only. Tuning changes must update deterministic fixtures and this document.
