...
The legacy recogniser and iOS 26 SpeechAnalyzer each retain their existing `AVAudioEngine` microphone tap. A selected audio or video presentation writes that tap's PCM buffers to a local CAF file while the same buffers continue to feed recognition and metering. Once an audio-only take stops, `AudioTakeExporter` uses Apple's audio-only M4A export preset and verifies an AAC track before the project store moves the result into Documents/Projects. A failed export preserves the original CAF as the take and alerts the user. The source CAF is removed only after a successful AAC take save. VideoCaptureCoordinator uses a camera-only session; it never installs another microphone input. After a video take ends, VideoTakeMuxer combines the camera movie and tap-recorded audio. This keeps AlignmentEngine and tracking instrumentation unchanged. AAC export and route behaviour still require physical iPhone checks.
