...
The meter maps approximately -60 dB to -12 dB RMS onto a zero-to-one scale and labels the result Quiet, Good or Too loud. It also measures the true sample peak and reports Clipping at 0.98 full scale or above. These thresholds are practical guidance rather than a guarantee of recognition accuracy. The optional microphone check samples ambient level without storing audio, adjusts the Quiet threshold in memory for that prompting session, and reports whether clipping occurred during the reading. Recording stops automatically when prompting is paused, backgrounded, interrupted or exited. Finished files are placed in the app's Documents/Recordings folder and can be exported with the system share sheet.
...
## Recording alongside recognition

The legacy recogniser and iOS 26 SpeechAnalyzer each retain their existing `AVAudioEngine` microphone tap. A selected audio or video presentation writes that tap's unprocessed PCM buffers to a local CAF file while the same buffers continue to feed recognition and metering. Once an audio-only take stops, `AudioTakeExporter` optionally renders an enhanced temporary PCM copy through an 80 Hz high-pass filter and gentle Apple dynamics processing, then uses Apple's audio-only M4A export preset and verifies an AAC track before the project store moves the result into Documents/Projects. Voice enhancement is off by default, never modifies the captured CAF, and never changes audio supplied to speech recognition. A failed enhancement or export preserves the original CAF as the take and alerts the user. The source CAF is removed only after a successful AAC take save. VideoCaptureCoordinator uses a camera-only session; it never installs another microphone input. After a video take ends, VideoTakeMuxer combines the camera movie and tap-recorded audio; the enhancement setting currently applies only to audio-only takes. This keeps AlignmentEngine and tracking instrumentation unchanged. Enhanced export, AAC export and route behaviour still require physical iPhone checks.
