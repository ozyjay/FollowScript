...
Presentation and earlier-recording rows offer deletion by a non-full-swipe trailing action or long-press menu, with a visible confirmation and Cancel. Take rows reserve the trailing swipe for immediate deletion so a full right-to-left swipe commits reliably; Share is available from a short leading swipe and the long-press menu. Deletion from the long-press menu still requires confirmation. The confirmation distinguishes removal of one recording from removal of a presentation and all its takes. The editor keeps Library in the top bar and groups Import and Settings in a More menu.
