...
Presentation and earlier-recording rows offer deletion by a non-full-swipe trailing action or long-press menu, with a visible confirmation and Cancel. Take rows support immediate deletion by a full trailing swipe; deletion from their long-press menu still requires confirmation. The confirmation distinguishes removal of one recording from removal of a presentation and all its takes. Recording rows also offer Share from either action surface. The editor keeps Library in the top bar and groups Import and Settings in a More menu.
...
Library rows now use their full width for Play and expose Share as a separate swipe action. `LocalRecordingFile` transfers CAF, M4A and MOV as typed files instead of bare file URLs, so large media need not be loaded into memory for export. `RecordingPlaybackModel` validates the local file before playback, configures the audio session for speaker output, uses `AVAudioPlayer` for CAF and M4A and `AVPlayer` for MOV, and reports media errors in the player sheet. Video playback overlays transparent controls on its edge-to-edge, aspect-fill `AVPlayerLayer`; audio playback retains its waveform and lower controls. No share action is presented from the playback sheet. The audio-session category is restored on exit; playback does not involve Speech or AlignmentEngine.
