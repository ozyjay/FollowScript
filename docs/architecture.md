...
`TeleprompterViewModel` owns session intent, recognition and level tasks, recording state, alignment state, errors and follow-suspension timing on the main actor. SwiftUI observes published state and decides only how to display it. Tracking-latency completion is coalesced in a cancellable main-actor task after state publication; it is not routed through an observable counter, avoiding multiple `onChange` mutations in one SwiftUI frame. `AppModel` owns the selected presentation and its editable script, remembers the selected presentation identifier, and persists settings in `UserDefaults`. Script edits are debounced briefly and written atomically through `PresentationProjectStore`; switching presentation or starting a presentation flushes pending edits first.

The native launch screen immediately presents the same dark foundation as the in-app loading experience, avoiding a bright blank transition while iOS starts the process. `RootView` then presents a brief in-app loading screen before the presentation Library, which is the application's main screen. The loading screen displays a presentation copy of the app icon and reads the current marketing version and Git-commit-count build number from the application bundle; it does not perform network or speech initialisation. The build-time mechanism is documented in `docs/versioning.md`.

`ScriptFileImporter` is the editor’s local document boundary. It reads security-scoped Files URLs, converts supported plain, attributed and ZIP-packaged XML documents to a `String`, then releases access. Only the resulting text crosses into `AppModel`, where it replaces the selected presentation's script; imported formatting and source-file access are not retained.
...
Presentation mode is chosen each time the script is presented and remembered in settings. Teleprompter mode keeps recognition in memory and creates no project or take. Audio and video modes create a project lazily when the user starts recording, then save each completed take as a media file with JSON metadata under Documents/Projects. The existing recognition service owns the only microphone tap; audio recording branches from that tap. Completed audio-only takes are exported from capture PCM to AAC in M4A after recording stops. The optional voice-enhancement setting renders a temporary high-pass-filtered and gently compressed PCM copy before AAC export; it does not alter live capture or recognition. If processing or export fails, the original CAF is saved as the take and a warning is shown. VideoCaptureCoordinator captures front-camera video without a microphone input, while VideoTakeMuxer joins that movie with the unprocessed tap-recorded audio after stopping. CameraPreview sits behind the SwiftUI prompt, so prompt text is not recorded. Enhanced audio and the post-recording video join need physical-device validation for sound quality, audio/video sync and interruptions.

## Saved presentation library

Presentations are the application's first-class document and Library is the root navigation screen. `AppModel` restores the last selected presentation on launch; on upgrade, it migrates the former standalone editor script once into a presentation named “My Presentation”. Creating a presentation in Library opens its editor. Selecting an existing presentation opens its detail, where the user can edit its script, start a presentation, manage its takes or rename it. The editor always shows and modifies the selected presentation's script, and the same presentation is passed into teleprompter, audio and video modes. Deleting the selected presentation chooses the next available presentation or creates an empty one.

`PresentationLibraryModel` lists project metadata, searches names and scripts, and exposes earlier standalone audio files. Project detail lists completed takes, supports playback and Files/share-sheet export, renaming, deletion, editing and starting another presentation. `PresentationProjectStore` owns JSON and media file operations; views do not write storage directly. The portable store tests use a temporary root to exercise creation, script updates, listing, renaming, take deletion and path validation.

Presentation and take rows reserve the trailing swipe for immediate deletion so a full right-to-left swipe commits reliably. Deleting a presentation this way also removes all its takes. Take sharing is available from a short leading swipe and the long-press menu. Long-press deletion still requires confirmation, as does deletion of earlier recordings. Destructive confirmations use centred system alerts that distinguish removal of one recording from removal of a presentation and all its takes. Library keeps New Presentation and Settings in its top bar; the editor's More menu contains presentation-specific Rename and Import actions.
