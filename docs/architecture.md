...
Presentations are the editor's first-class document. `AppModel` restores the last selected presentation on launch; on upgrade, it migrates the former standalone editor script once into a presentation named “My Presentation”. The editor always shows and modifies the selected presentation's script, its title appears in the navigation bar, and the same presentation is passed into teleprompter, audio and video modes. Users can create or rename presentations from the editor, create one from Library, import a script into the selected presentation, or choose Edit Presentation from project detail. Deleting the selected presentation chooses the next available presentation or creates an empty one.
