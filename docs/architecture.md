...
Presentation, take and earlier-recording rows offer deletion by a non-full-swipe trailing action or long-press menu; each requires a visible confirmation with Cancel. The confirmation distinguishes removal of one recording from removal of a presentation and all its takes. Recording rows also offer Share from either action surface. The editor keeps Library in the top bar and groups Import and Settings in a More menu.

## Local take playback

Library rows now use their full width for Play and expose Share as a separate swipe action. `LocalRecordingFile` transfers CAF and MOV as typed files instead of bare file URLs, so large media need not be loaded into memory for export. `RecordingPlaybackModel` validates the local file before playback, configures the audio session for speaker output, uses `AVAudioPlayer` for CAF and `AVPlayer` for MOV, and reports media errors in the player sheet. The video surface is an `AVPlayerLayer` with app controls, avoiding native video-player sharing inside an already presented sheet. No share action is presented from the playback sheet. The audio-session category is restored on exit; playback does not involve Speech or AlignmentEngine.
